package sefa.probe.rest;

import sefa.probe.configuration.ServiceConfiguration;
import sefa.probe.domain.InstanceStatus;
import sefa.probe.domain.ProbeService;
import sefa.probe.domain.ServiceInfo;
            ...
            synchronized (lock) {
                if (fakeCounter > 0) {
                    if (instanceToMakeUnreachable == null) {
                        ...
                        log.debug(serviceId+ "SETTLED AS UNREACHABLE");
                    } else {
