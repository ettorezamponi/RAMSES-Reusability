...
public interface MetricsRepository extends CrudRepository<InstanceMetricsSnapshot, Long> {
    ...
    String SHUTDOWNSTATUS = "ramses.knowledge.domain.architecture.InstanceStatus.SHUTDOWN";
    ...
    @Query("SELECT m FROM InstanceMetricsSnapshot m WHERE m.instanceId = :instanceId AND " +
            "m.timestamp > IFNULL((SELECT MAX(m1.timestamp) FROM InstanceMetricsSnapshot m1 WHERE m1.instanceId = :instanceId AND m.status="+SHUTDOWNSTATUS+"), ramses.knowledge.domain.persistence.MetricsRepository.MIN_TIMESTAMP) " +
    ...
    @Query("SELECT m FROM InstanceMetricsSnapshot m WHERE m.instanceId = :instanceId AND m.status=ramses.knowledge.domain.architecture.InstanceStatus.ACTIVE " +
